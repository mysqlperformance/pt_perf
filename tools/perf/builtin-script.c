...
enum perf_output_field {
	...
	PERF_OUTPUT_RETIRE_LAT      = 1ULL << 40,
        PERF_OUTPUT_THREAD_PERIOD   = 1ULL << 41, 
};
...
struct output_option {
	...
	{.str = "thread_period", .field = PERF_OUTPUT_THREAD_PERIOD},
};
			  ...
			  struct machine *machine)
{
	...
	perf_sample__fprintf_start(script, sample, thread, evsel,
        ...
        if (PRINT_FIELD(THREAD_PERIOD)) {
                if (!thread->last_sample_ts) {
                	fprintf(fp, "%10" PRIu64 " ", 0ul);
                } else {
			fprintf(fp, "%10" PRIu64 " ", sample->time - thread->last_sample_ts);
                }
                thread->last_sample_ts = sample->time;
        } else if (PRINT_FIELD(PERIOD))
...
int cmd_script(int argc, const char **argv)
{
	...
	OPT_STRING(0, "func_filter", &func_filter, "func_filter",
		   "only decode specified function"),
